...
- `read_statement_declaration` (statement_declaration.c) — Declaration statement (`[export|extern|builtin|comptime|using] var <declarator> ;`). Parses a single variable declarator: `<identifier> [: <type>] [= <expression>]`. Modifiers: `export` (exported from module), `extern` (external linkage, no initializer, requires type annotation), `builtin` (compiler-provided, no initializer, requires type annotation), `comptime` (compile-time evaluated, requires initializer, mutually exclusive with `extern` and `builtin`), `using` (RAII declaration, auto-defer `__dispose__` at scope exit, mutually exclusive with `extern`, `builtin`, and `comptime`, not allowed at module scope, cannot use `undefined`). `export` and `builtin` are orthogonal (can combine). `export` and `comptime` are orthogonal (can combine). `export` and `using` are orthogonal (can combine). `extern` is mutually exclusive with `export`, `builtin`, `comptime`, and `using`. `builtin` and `comptime` are mutually exclusive. `using` is mutually exclusive with `builtin`, `comptime`, and `extern`. Extern/builtin declarations must not have `= expression`. Comptime declarations must have `= expression`. Using declarations must have `= expression` (not `undefined`). Returns `cubec_statement_declaration_t` with `is_export`, `is_extern`, `is_builtin`, `is_comptime`, `is_using` (bools) and `declarator` (single `declaration_variable_t` node) fields.
