...
> 基于 `src/cubec/` 中每个 AST 节点自带的 `emit_*` 函数整理。
>
> **后端实际位置**：C 后端代码生成**不是**集中式的 `src/writer/` 模块
> （该目录不存在），而是**分发式**的——`src/cubec/` 下每个 AST 节点文件
> （`statement_*.c`、`expression_*.c`、`declaration_*.c`、`literal_*.c`、
> `program.c`）都包含自己的 `emit_*` 函数，经由 `emit_context_t` 输出到
> `core/token_writer`。顶层入口为 `emit_program`。

## 1. 渐进式多后端

Cubec 采用渐进式多后端策略，当前仅实现 C 后端，LLVM 后端尚未开始。

| 后端 | 状态 | 优势 | 劣势 |
|------|------|------|------|
| C | 已实现 | 复用 C 编译器工具链，快速验证语义 | 依赖外部 C 编译器 |
| LLVM | 待实现 | 利用 LLVM 优化和生态 | 依赖 LLVM |

---

## 2. C 后端目标

输出 `.c` 文件（以及可能的 `.h` 头文件），由 C 编译器（clang/gcc）编译为最终产物。
...
> **注意**：早期文档描述的 `m3a7_` 模块哈希前缀在**当前源码中未找到对应实现**
> （`src/cubec/` 的 `emit_*` 函数里不存在 `m3a7_` 字符串）。当前 C 后端的实际
> 名称修饰策略可能尚未实现或采用不同方案，本节保留早期设计作为参考，但以下映射
> 规则（`.` → `_`、嵌套用 `__`、泛型实例含类型参数等）的方向仍可作理解。
