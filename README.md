...
- **语义分析引擎**：双层类型表示、结构等价、指针退化、TDZ 多遍检查、const/volatile 语义、builtin 回调注册表、原生 tuple 类型（`<i32, f64>` 语法）、TYPE_OPAQUE（对标 C `void*`）、参数包（variadic generics）、泛型推断、匿名 initialize_list 推断、struct-like 隐式转换、显式类型转换 cast builtin、`<?>` 元组通配符约束、Rustc 风格诊断、`undefined` 字面量与强制变量初始化、表达式语句返回值未使用警告
