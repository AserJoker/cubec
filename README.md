...
`typeof` 在语法上被视为**类型表达式**，可以用于任何需要类型的地方：

```c
typeof(File)::open     // typeof 结果 + 命名空间访问（类型静态方法）
* typeof(x)            // 指向 typeof 结果的指针
[] typeof(x)           // typeof 结果的切片
typeof(Vec)[i32]       // typeof 结果的泛型实例化
```

> **注意**：`typeof` 仅在编译期计算类型，内部表达式不会在运行时求值。与三元表达式不同，`typeof` 可以直接作为指针、切片声明的 base_type。
...
├── test/             # 测试文件（Google Test, 589 测试用例）
├── demo/             # 示例 .cubec 文件
└── third_party/      # 第三方依赖
```
...
- **表达式解析器**：
  ...
  - typeof 表达式（`typeof(<expression>)`，编译期类型计算，语法上视为类型表达式，可作为指针/切片的 base_type，支持后缀 `::`/`[]`/`*`）
