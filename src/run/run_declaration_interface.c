...
/* ---- helper: add members (method signatures) to an interface type value ---- */
...
/* ---- main entry: anonymous interface type expression ---- */
...
value_t run_declaration_interface(vm_t vm, node_t node, bool shadow) {
  ...
  if (decl->generic_params && vec_get_size(decl->generic_params) > 0) {
    ...
    /* create generic type — node is the declaration_interface_t (borrowed) */
  ...
  /* ---- non-generic interface expression ---- */

  /* create the interface type value (unsealed, anonymous = NULL name) */
  value_t type_val = vm_create_interface_type_value(vm, NULL, false,
                          vm_get_current_module_id(vm));

  /* add method signatures */
  value_t members_result = _add_interface_members(vm, type_val, decl->members);
