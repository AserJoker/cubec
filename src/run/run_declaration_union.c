...
/* ---- main entry: anonymous union type expression ---- */
...
value_t run_declaration_union(vm_t vm, node_t node, bool shadow) {
  ...
  if (decl->generic_params && vec_get_size(decl->generic_params) > 0) {
    ...
    /* create generic type — node is the declaration_union_t (borrowed) */
  ...
  /* ---- non-generic union expression ---- */

  /* create the union type value (unsealed, anonymous = NULL name) */
  value_t type_val = vm_create_union_type_value(vm, NULL, false,
                          vm_get_current_module_id(vm));

  /* add members (fields, methods, props) */
  value_t members_result = _add_union_members(vm, type_val, decl->members);
