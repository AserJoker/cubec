...
cubec_write_ast_function_declarator(cubec_allocator_t allocator,
  ...
  cubec_any_t closure = cubec_create_any(allocator);
  cubec_any_set_array(closure, allocator);
  it = cubec_list_get_first(self->closure);
  while (it != cubec_list_get_end(self->closure)) {
    cubec_ast_node_t item = cubec_list_node_get(it);
    cubec_any_append(closure, allocator, cubec_write_ast_node(item, allocator));
    it = cubec_list_node_next(it);
  }
  cubec_any_set_field(value, allocator, "closure", closure);
  if (self->identifier) {
