...
static void _cubec_expression_deref_dispose(cubec_expression_deref_t self,
                                            allocator_t allocator) {
  allocator_free(allocator, &self->host);
  g_cubec_expression_class.dispose(&self->super, allocator);
}

static void _cubec_expression_deref_clone(cubec_expression_deref_t self,
                                          allocator_t allocator,
                                          cubec_expression_deref_t another) {
  g_cubec_expression_class.clone(&self->super, allocator, &another->super);
  self->host = alloc_clone(allocator, another->host);
}

static void _cubec_expression_deref_move(cubec_expression_deref_t self,
                                         allocator_t allocator,
                                         cubec_expression_deref_t another) {
...
class_t g_cubec_expression_deref_class = {
    .name = "cubec.cubec.expression_deref",
    .size = sizeof(struct _cubec_expression_deref_t),
    .init = (class_init_fn_t)_cubec_expression_deref_init,
    .dispose = (class_dispose_fn_t)_cubec_expression_deref_dispose,
    .clone = (class_clone_fn_t)_cubec_expression_deref_clone,
    .move = (class_move_fn_t)_cubec_expression_deref_move,
};

/* --------------------------------------------------------------------------
 *  Parser: read_expression_deref
 * -------------------------------------------------------------------------- */

/**
 * Try to parse postfix dereference: <host>.*
...
node_t read_expression_deref(vm_t vm, vec_t tokens, size_t *position,
  ...
  cubec_expression_deref_t node = NULL;
  ...
  node = allocator_create(allocator, &g_cubec_expression_deref_class,
                          &(cubec_expression_deref_init_t){
                              .host = host,
                          });
...
/* --------------------------------------------------------------------------
 *  Factory: create_expression_deref
 * -------------------------------------------------------------------------- */
...
node_t create_expression_deref(vm_t vm, location_t loc, node_t host) {
  ...
  return (node_t)allocator_create(alloc, &g_cubec_expression_deref_class, &init);
}
