...
class_t g_cubec_statement_union_class = {
    .name = "cubec.cubec.statement_union",
    .size = sizeof(struct _cubec_statement_union_t),
    .init = (class_init_fn_t)_cubec_statement_union_init,
    .dispose = (class_dispose_fn_t)_cubec_statement_union_dispose,
    .clone = (class_clone_fn_t)_cubec_statement_union_clone,
    .move = (class_move_fn_t)_cubec_statement_union_move,
};
...
node_t read_statement_union(vm_t vm, vec_t tokens, size_t *position,
                            const char *filename) {
   ...
   * members } */
  expr_node = read_declaration_union_body(vm, tokens, &current, filename,
  ...
  node = allocator_create(allocator, &g_cubec_statement_union_class, &init);
  *position = current;
  return &node->super;
...
node_t create_statement_union(vm_t vm, location_t loc, const char *name,
  ...
  return (node_t)allocator_create(alloc, &g_cubec_statement_union_class, &init);
}
