...
node_t read_statement_while(allocator_t allocator, vec_t tokens,
  ...
  token_t while_token = TRY_LOCAL(onerror, vec_get(tokens, current));
  ...
  if (!_is_symbol(tokens, current, "(")) {
    THROW_LOCAL(onerror, "expected '(' after 'while'");
  }
  current++;
  skip_whitespace(tokens, &current);
  ...
  if (!body) {
    THROW_LOCAL(cleanup, "expected statement after while condition");
  }
  ...
  node = TRY_LOCAL(cleanup, allocator_create(allocator, &g_cubec_statement_while_type, &init));
  *position = current;
  return &node->super;
