...
node_t read_expression_alignof(allocator_t allocator, vec_t tokens,
  ...
  token_t alignof_token = TRY_LOCAL(onerror, vec_get(tokens, current));
  ...
  if (!token_is(lparen, CUBEC_TOKEN_SYMBOL, "(")) {
    ...
    THROW_LOCAL(onerror,
                "%s:%" PRIuPTR ":%" PRIuPTR " expected '(' after 'alignof'",
  ...
  if (!expr) {
    THROW_LOCAL(onerror, "%s: expected expression inside alignof()", filename);
  }
  ...
  if (!token_is(rparen, CUBEC_TOKEN_SYMBOL, ")")) {
    ...
    THROW_LOCAL(cleanup,
                "%s:%" PRIuPTR ":%" PRIuPTR " expected ')' to close alignof()",
                filename, loc->begin.line + 1, loc->begin.column);
  }
  current++;
  ...
  cubec_expression_alignof_t node =
      TRY_LOCAL(cleanup, allocator_create(allocator, &g_cubec_expression_alignof_type, &init));
  *position = current;
