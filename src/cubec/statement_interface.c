 ...
 * read_expression_type_interface
 * -------------------------------------------------------------------------- */
...
node_t read_statement_interface(context_t ctx, vec_t tokens, size_t *position,
  ...
  /* 4. Delegate to read_expression_type_interface_body for [generic_params] {
   * members } (interface keyword already consumed, pass start_location for
   * span) */
  expr_node = read_expression_type_interface_body(ctx, tokens, &current,
  ...
  if (!expr_node)
    goto onerror;
  cubec_expression_type_interface_t expr_iface =
      (cubec_expression_type_interface_t)expr_node;
