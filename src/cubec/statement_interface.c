...
node_t read_statement_interface(allocator_t allocator, vec_t tokens,
  ...
  if (!name) {
    THROW_LOCAL(cleanup, "expected interface name after 'interface'");
  }
  ...
  expr_node = TRY_LOCAL(cleanup, read_expression_type_interface_body(allocator, tokens, &current, filename, start_location));
  if (!expr_node) {
    THROW_LOCAL(cleanup, "expected '{' after interface name");
  }
  ...
  node = TRY_LOCAL(cleanup, allocator_create(allocator, &g_cubec_statement_interface_type, &init));
  *position = current;
  return &node->super;
