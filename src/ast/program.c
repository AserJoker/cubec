...
cubec_ast_program_t cubec_create_ast_program(cubec_allocator_t allocator) {
  ...
  cubec_ast_set_field(program, allocator, statements);
  program->statements = cubec_create_ast_list_node(allocator);
  return program;
}
...
cubec_ast_node_t cubec_read_ast_program(cubec_allocator_t allocator,
  ...
  for (;;) {
    ...
    cubec_ast_list_node_append(program->statements, allocator, stat);
  ...
  *position = current;
  cubec_ast_set_parent(program->statements, &program->super);
