...
ast_node_t read_statement_if(allocator_t allocator, token_stream_t stream) {
  ast_node_t node = create_ast_node(allocator, NODE_TYPE_STATEMENT_IF);
  ast_node_t err = NULL;
  size_t position = stream->position;
  token_t token = token_stream_get(stream);
  if (!token_is(token, TOKEN_TYPE_KEYWORD, "if")) {
    goto onerror;
  }
  stream->position++;
  ...
  stream->position++;
  skip_comments(stream);
  ast_node_t consequent = read_statement(allocator, stream);
  if (!consequent) {
    token_t start = array_get(stream->tokens, position);
    token_t end = token_stream_get(stream);
    err = create_ast_error(allocator, start->loc.begin, end->loc.end,
                           stream->filename, "expected expression");
    goto onerror;
  }
  ...
  skip_comments(stream);
  token = token_stream_get(stream);
  if (token_is(token, TOKEN_TYPE_KEYWORD, "else")) {
    stream->position++;
    skip_comments(stream);
    ast_node_t alternate = read_statement(allocator, stream);
    if (!alternate) {
      token_t start = array_get(stream->tokens, position);
      token_t end = token_stream_get(stream);
      err = create_ast_error(allocator, start->loc.begin, end->loc.end,
