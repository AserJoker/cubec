...
cubec_create_ast_statement_break(cubec_allocator_t allocator) {
  cubec_ast_statement_break_t self = cubec_allocator_alloc(
      allocator, sizeof(struct _cubec_ast_statement_break_t),
      (cubec_dispose_fn_t)cubec_ast_statement_break_dispose);
  cubec_ast_node_initialize(allocator, &self->super);
  self->super.type = CUBEC_NODE_TYPE_STATEMENT_BREAK;
  return self;
}
cubec_ast_node_t cubec_read_ast_statement_break(cubec_allocator_t allocator,
                                                cubec_position_t *position,
                                                const char *end) {
  cubec_ast_statement_break_t node =
      cubec_create_ast_statement_break(allocator);
