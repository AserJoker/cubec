...
static void _map_init(map_t self, allocator_t allocator, map_init_t *init) {
  ...
  for (size_t i = 0; i < DEFAULT_BUCKET_COUNT; i++) {
    ...
    self->index.buckets[i] = allocator_create(allocator, &g_list_class, &list_init);
  }
}
...
static void _map_move(map_t self, allocator_t allocator, map_t another) {
  ...
  another->keys = allocator_create(allocator, &g_vec_class, &(vec_init_t){false});
  another->values = allocator_create(allocator, &g_vec_class, &(vec_init_t){false});
  another->index_type = MAP_INDEX_HASH;
  for (size_t idx = 0; idx < DEFAULT_BUCKET_COUNT; idx++) {
    list_init_t list_init = {.auto_dispose = true};
    another->index.buckets[idx] = allocator_create(allocator, &g_list_class, &list_init);
  }
}
...
class_t g_map_class = {
    ...
    .name = "cubec.core.map",
    .init = (class_init_fn_t)_map_init,
    .dispose = (class_dispose_fn_t)_map_dispose,
    .clone = (class_clone_fn_t)_map_clone,
    .move = (class_move_fn_t)_map_move,
};
