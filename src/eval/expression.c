...
cubec_value_t cubec_eval_expression(cubec_context_t ctx, cubec_ast_node_t expr,
  ...
  if (expr->type == CUBEC_NODE_TYPE_EXPRESSION_BINARY) {
    return cubec_eval_expression_binary(
        ctx, (cubec_ast_expression_binary_t)expr, filename);
  } else if (expr->type == CUBEC_NODE_TYPE_LITERAL_IDENTIFIER) {
    return cubec_eval_literal_identifier(
        ctx, (cubec_ast_literal_identifier_t)expr, filename);
  } else if (expr->type == CUBEC_NODE_TYPE_LITERAL_STRING) {
    return cubec_eval_literal_string(ctx, (cubec_ast_literal_string_t)expr,
                                     filename);
  } else if (expr->type == CUBEC_NODE_TYPE_PTR_DECLARATOR) {
    return cubec_eval_ptr_declarator(ctx, (cubec_ast_ptr_declarator_t)expr,
                                     filename);
  } else if (expr->type == CUBEC_NODE_TYPE_EXPRESSION_MEMBER) {
    return cubec_eval_expression_member(
        ctx, (cubec_ast_expression_member_t)expr, filename);
  } else if (expr->type == CUBEC_NODE_TYPE_EXPRESSION_CALL) {
    return cubec_eval_expression_call(ctx, (cubec_ast_expression_call_t)expr,
                                      filename);
  } else {
