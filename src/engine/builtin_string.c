 ...
 *        This is a safe operation — it never fails and always produces a
 * string.
 *
...
static comptime_value_t _to_string_value(struct comptime_eval *eval,
  ...
  case COMPTIME_VALUE_BOOL:
    return _eval_temp(eval,
                      comptime_value_create_literal_string(
                          eval->allocator, arg->bool_val ? "true" : "false",
                          ctx->builtin_str));
    ...
    if (arg->type) {
      ...
      if (arg->composite.element_type) {
        size_t elem_size = arg->composite.element_type->impl
                               ? arg->composite.element_type->impl->size
                               : 0;
        size_t count = (elem_size > 0 && arg->composite.data_size >= elem_size)
                           ? arg->composite.data_size / elem_size
                           : 0;
        string_t builder =
            (string_t)allocator_create(eval->allocator, &g_string_type, NULL);
        for (size_t i = 0; i < count; i++) {
          if (i > 0)
            string_concat(builder, ", ");
          comptime_value_t elem = _eval_temp(
              eval, comptime_value_get_index(arg, i, eval->allocator));
          if (elem && elem->kind != COMPTIME_VALUE_ERROR) {
            comptime_value_t elem_str =
                _to_string_value(eval, ctx, elem, node_for_loc);
        ...
        comptime_value_t result = _eval_temp(
            eval, comptime_value_create_literal_string(
      ...
      /* struct/union/cunion without toString: {struct TypeName 0xADDR} */
      {
        ...
        const char *type_name =
            arg->type->name ? arg->type->name : "<anonymous>";
  ...
  case COMPTIME_VALUE_TYPE:
    return _eval_temp(eval, comptime_value_create_literal_string(
                                eval->allocator,
                                arg->type_val && arg->type_val->name
                                    ? arg->type_val->name
                                    : "<type>",
                                ctx->builtin_str));
  ...
  default:
    return _eval_temp(
        eval, comptime_value_create_literal_string(eval->allocator, "<unknown>",
                                                   ctx->builtin_str));
  }
}
