...
vec_t _resolve_func_param_types(context_t ctx, const func_check_info_t *info) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    semantic_type_t pt =
        farg->type ? resolver_resolve_type(ctx, farg->type) : ctx->error_type;
...
void _register_func_params_from_info(context_t ctx,
  ...
  for (size_t j = 0; j < count; j++) {
    ...
    if (!pname)
      continue;
    struct symbol *psym =
        symbol_create(ctx->allocator, pname, SYMBOL_VARIABLE, arg->location);
  ...
  for (size_t i = 0; i < cc; i++) {
    ...
    if (outer_sym) {
      ...
      if (outer_sym->state == SYMBOL_TDZ &&
          (!ctx->current_flow ||
           flow_state_is_tdz(ctx->current_flow, cap_name))) {
        diagnostic_list_push(
            ctx->diagnostics, DIAGNOSTIC_ERROR, cap_node->location,
            "cannot capture variable '%s' before initialization", cap_name);
        ctx->error_count++;
      }
      struct symbol *cap_sym = symbol_create(
          ctx->allocator, cap_name, outer_sym->kind, cap_node->location);
...
void _check_func_body_and_returns(context_t ctx, const func_check_info_t *info,
                                  semantic_type_t return_type,
                                  vec_t param_types, scope_t scope_root) {
  if (!info->body)
    return;

  scope_t saved = ctx->current_scope;
  ctx->current_scope =
      scope_create(ctx->allocator, scope_root, SCOPE_FUNCTION, info->location);
  ...
  if (return_type && return_type->impl->kind != TYPE_VOID &&
    ...
    diagnostic_list_push(ctx->diagnostics, DIAGNOSTIC_ERROR, info->location,
...
void context_register_generic_params(context_t ctx, vec_t generic_params) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!gp_node || gp_node->kind != CUBEC_NODE_GENERIC_PARAM)
      continue;
    ...
    if (gp->is_rest) {
      ...
      if (i != count - 1) {
        diagnostic_list_push(
            ctx->diagnostics, DIAGNOSTIC_ERROR, gp->super.location,
    ...
    struct symbol *sym = symbol_create(
        ctx->allocator, gp_name, SYMBOL_GENERIC_PARAM, gp->super.location);
    ...
    if (gp->value_type)
      sym->generic_param.value_type =
          resolver_resolve_type(ctx, gp->value_type);
...
semantic_type_t _process_function(context_t ctx, func_check_info_t *info,
  ...
  if (info->generic_params) {
    if (fctx->use_child_scope) {
      ctx->current_scope =
          scope_create(ctx->allocator, saved, SCOPE_BLOCK, info->location);
      vec_push(ctx->all_scopes, ctx->current_scope);
    }
