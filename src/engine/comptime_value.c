...
static void _foreach_string_field(comptime_value_t v,
                                  ...
                                  void *ctx) {
  if (!v->type || !v->type->impl || !v->composite.data)
    return;
  vec_t fields = NULL;
  ...
  for (size_t i = 0; i < fc; i++) {
    ...
    if (!s || (s->kind != SYMBOL_VARIABLE && s->kind != SYMBOL_FIELD))
      continue;
    ...
    if (is_union && active_tag != 0) {
      type_hash_ensure(ft);
      if (ft->impl->hash != active_tag)
        continue;
    }
    enum type_kind fk = ft->impl->kind;
    if (fk == TYPE_STRING || fk == TYPE_STR) {
      string_t *slot =
          (string_t *)(void *)(v->composite.data + s->field.offset);
      fn(slot, ctx);
    }
  }
}
...
static bool _clone_string_slot(string_t *slot, void *ctx) {
  ...
  if (*slot) {
    string_t new_str =
        (string_t)allocator_create(allocator, &g_string_type, NULL);
...
static void _comptime_value_dispose(void *self, allocator_t allocator) {
  ...
  case COMPTIME_VALUE_COMPOSITE:
    if (v->composite.data) {
      /* Dispose string_t objects embedded in composite data before freeing the
       * buffer. For tagged unions, only the active variant's string_t is valid.
       */
  ...
  *dst = *src;
  ...
  case COMPTIME_VALUE_COMPOSITE:
    if (src->composite.data && src->composite.data_size > 0) {
      dst->composite.data =
          (uint8_t *)allocator_alloc(allocator, src->composite.data_size);
      memcpy(dst->composite.data, src->composite.data,
  ...
  case COMPTIME_VALUE_PACK:
    if (src->pack.elements) {
      ...
      dst->pack.elements = (vec_t)allocator_create(allocator, &g_vec_type, &vi);
      for (size_t i = 0; i < ec; i++) {
        comptime_value_t elem =
            (comptime_value_t)vec_get(src->pack.elements, i);
...
comptime_value_t comptime_value_create_int(allocator_t allocator, int64_t sval,
  ...
  struct comptime_value init = {
      ...
      .type = type,
      .int_val = {
          .s = sval, .u = uval, .width = width, .is_signed = is_signed}};
  return (comptime_value_t)allocator_create(allocator, &g_comptime_value_type,
                                            &init);
}
...
comptime_value_t comptime_value_create_float(allocator_t allocator, double val,
                                             uint8_t width,
                                             semantic_type_t type) {
  struct comptime_value init = {.kind = COMPTIME_VALUE_FLOAT,
                                .type = type,
...
comptime_value_t comptime_value_create_composite(allocator_t allocator,
  ...
  struct comptime_value init = {.kind = COMPTIME_VALUE_COMPOSITE,
                                .type = type,
                                .composite = {.data = data,
                                              .data_size = data_size,
...
comptime_value_t comptime_value_create_function(allocator_t allocator,
                                                ...
                                                vec_t param_names,
                                                semantic_type_t type) {
  struct comptime_value init = {.kind = COMPTIME_VALUE_FUNCTION,
                                .type = type,
...
comptime_value_t comptime_value_create_pack(allocator_t allocator,
                                            vec_t elements,
                                            semantic_type_t type) {
  struct comptime_value init = {.kind = COMPTIME_VALUE_PACK,
                                .type = type,
...
bool comptime_value_is_truthy(comptime_value_t val) {
  ...
  case COMPTIME_VALUE_INT:
    return val->int_val.is_signed ? val->int_val.s != 0 : val->int_val.u != 0;
  case COMPTIME_VALUE_FLOAT:
    return val->float_val.value != 0.0;
  case COMPTIME_VALUE_CHAR:
    return val->char_val != 0;
  case COMPTIME_VALUE_STRING: {
    const char *s = val->string_val ? string_get(val->string_val) : NULL;
    return s && s[0] != '\0';
  }
  case COMPTIME_VALUE_POINTER:
    return val->pointer.addr != 0;
  default:
    return true;
  }
}
...
bool comptime_value_equals(comptime_value_t a, comptime_value_t b) {
  ...
  case COMPTIME_VALUE_INT:
    ...
    if (a->int_val.is_signed && a->int_val.s < 0)
      return false;
    if (b->int_val.is_signed && b->int_val.s < 0)
      return false;
  ...
  case COMPTIME_VALUE_POINTER:
    return a->pointer.addr == b->pointer.addr;
  case COMPTIME_VALUE_COMPOSITE: {
    if (a->composite.data_size != b->composite.data_size)
      return false;
    if (!a->composite.data && !b->composite.data)
      return true;
    if (!a->composite.data || !b->composite.data)
      return false;
    ...
    if (a->composite.element_type &&
        a->composite.element_type == b->composite.element_type) {
      return memcmp(a->composite.data, b->composite.data,
                    a->composite.data_size) == 0;
    }
  ...
  case COMPTIME_VALUE_PACK: {
    ...
    for (size_t i = 0; i < ac; i++) {
      ...
      if (!comptime_value_equals(ea, eb))
        return false;
    }
    return true;
  }
  case COMPTIME_VALUE_ERROR:
    return true;
  default:
    return false;
  }
...
int64_t comptime_value_as_i64(comptime_value_t val) {
  ...
  switch (val->kind) {
  case COMPTIME_VALUE_INT:
    return val->int_val.is_signed ? val->int_val.s : (int64_t)val->int_val.u;
  case COMPTIME_VALUE_FLOAT:
    return (int64_t)val->float_val.value;
  case COMPTIME_VALUE_BOOL:
    return val->bool_val ? 1 : 0;
  case COMPTIME_VALUE_CHAR:
    return (int64_t)val->char_val;
  default:
    return 0;
  }
}
...
uint64_t comptime_value_as_u64(comptime_value_t val) {
  ...
  switch (val->kind) {
  case COMPTIME_VALUE_INT:
    return val->int_val.is_signed ? (uint64_t)val->int_val.s : val->int_val.u;
  case COMPTIME_VALUE_FLOAT:
    return (uint64_t)val->float_val.value;
  case COMPTIME_VALUE_BOOL:
    return val->bool_val ? 1 : 0;
  case COMPTIME_VALUE_CHAR:
    return (uint64_t)val->char_val;
  default:
    return 0;
  }
}
...
uint64_t comptime_value_get_union_tag(comptime_value_t comp) {
  if (!comptime_value_is_tagged_union(comp))
    return 0;
  if (!comp->composite.data || comp->composite.data_size < 8)
    return 0;
  uint64_t tag;
...
comptime_value_t comptime_value_read_field(comptime_value_t composite,
  ...
  case TYPE_U8:
    return comptime_value_create_int(allocator, (int64_t)(*(uint8_t *)ptr),
                                     (uint64_t)(*(uint8_t *)ptr), 8, false,
                                     field_type);
  case TYPE_U16:
    return comptime_value_create_int(allocator, (int64_t)(*(uint16_t *)ptr),
                                     (uint64_t)(*(uint16_t *)ptr), 16, false,
                                     field_type);
  case TYPE_U32:
    return comptime_value_create_int(allocator, (int64_t)(*(uint32_t *)ptr),
                                     (uint64_t)(*(uint32_t *)ptr), 32, false,
                                     field_type);
  case TYPE_U64:
    return comptime_value_create_int(allocator, (int64_t)(*(uint64_t *)ptr),
                                     *(uint64_t *)ptr, 64, false, field_type);
  case TYPE_F32:
    return comptime_value_create_float(allocator, (double)(*(float *)ptr), 32,
                                       field_type);
  case TYPE_F64:
    return comptime_value_create_float(allocator, *(double *)ptr, 64,
                                       field_type);
  case TYPE_CHAR:
    return comptime_value_create_literal_char(allocator, *(char *)ptr,
                                              field_type);
  case TYPE_POINTER:
    return comptime_value_create_pointer(allocator, *(uint64_t *)ptr,
                                         field_type);
  case TYPE_STRING:
  case TYPE_STR: {
    /* string_t stored as pointer in 8 bytes */
    string_t s = *(string_t *)(void *)ptr;
    return comptime_value_create_literal_string(
        allocator, s ? string_get(s) : NULL, field_type);
  }
  default:
    return NULL;
  }
...
bool comptime_value_write_field(comptime_value_t composite, size_t offset,
  ...
  case COMPTIME_VALUE_STRING: {
     ...
     * then store the new pointer. This avoids sharing string_t between
     * composites, which would cause use-after-free during disposal. */
  ...
  case COMPTIME_VALUE_COMPOSITE:
    if (value->composite.data && value->composite.data_size > 0) {
      ...
      if (field_size > 0 && copy_sz > field_size)
        copy_sz = field_size;
...
comptime_value_t comptime_value_get_field(comptime_value_t composite,
                                          ...
                                          allocator_t allocator) {
  if (!composite || !composite->type || !field_name)
    return NULL;
...
bool comptime_value_set_field(comptime_value_t composite,
  ...
  for (size_t i = 0; i < fc; i++) {
    struct symbol *s = (struct symbol *)vec_get(fields, i);
    if (s && s->name && strcmp(s->name, field_name) == 0)
      return comptime_value_write_field(composite, s->field.offset,
                                        s->field.type, value, allocator);
  }
  return false;
}
...
comptime_value_t comptime_value_get_index(comptime_value_t composite,
  ...
  if (!composite || !composite->composite.data)
    return NULL;
  ...
  if (offset + elem_size > composite->composite.data_size)
    return NULL;
