...
static comptime_value_t _create_truncated_int(struct comptime_eval *eval,
  ...
  bool is_signed = (to_unq->impl->kind >= TYPE_I8 &&
                    to_unq->impl->kind <= TYPE_I64);
  ...
  return comptime_value_create_int(eval->allocator, sval, uval, width, is_signed, to);
}
...
static comptime_value_t _cast_numeric(struct comptime_eval *eval,
  ...
  if (fk == TYPE_BOOL && tk >= TYPE_I8 && tk <= TYPE_U64) {
    ...
    return _eval_temp(eval, comptime_value_create_int(eval->allocator,
        (int64_t)uval, uval, width, is_signed, to));
  }
  ...
  if (fk >= TYPE_I8 && fk <= TYPE_U64 && tk == TYPE_CHAR) {
    ...
    return _eval_temp(eval, comptime_value_create_char(eval->allocator, c, to));
  }
...
static comptime_value_t _cast_pointer(struct comptime_eval *eval,
  ...
  if (from_unq->impl->kind == TYPE_OPAQUE && to_unq->impl->kind == TYPE_POINTER) {
    ...
    return _eval_temp(eval, comptime_value_create_pointer(eval->allocator, addr, to));
  }
...
static comptime_value_t _cast_container(struct comptime_eval *eval,
  ...
  if (from_unq->impl->kind == TYPE_ARRAY && to_unq->impl->kind == TYPE_TUPLE) {
    ...
    if (result && src_val->composite.data && src_val->composite.data_size > 0)
      memcpy(result->composite.data, src_val->composite.data, src_val->composite.data_size);
...
struct comptime_value *builtin_cast_eval(struct comptime_eval *eval,
  ...
  if (semantic_type_can_implicit_convert(source_type, target_type)) {
    ...
    case COMPTIME_VALUE_INT:
      return _eval_temp(eval, comptime_value_create_int(eval->allocator,
          src_val->int_val.s, src_val->int_val.u,
          src_val->int_val.width, src_val->int_val.is_signed, target_type));
    case COMPTIME_VALUE_FLOAT:
      return _eval_temp(eval, comptime_value_create_float(eval->allocator,
          src_val->float_val.value, src_val->float_val.width, target_type));
    case COMPTIME_VALUE_BOOL:
      return _eval_temp(eval, comptime_value_create_bool(eval->allocator,
          src_val->bool_val, target_type));
    case COMPTIME_VALUE_CHAR:
      return _eval_temp(eval, comptime_value_create_char(eval->allocator,
          src_val->char_val, target_type));
    case COMPTIME_VALUE_POINTER:
      return _eval_temp(eval, comptime_value_create_pointer(eval->allocator,
          src_val->pointer.addr, target_type));
    default:
  ...
  diagnostic_list_push(ctx->diagnostics, DIAGNOSTIC_ERROR,
                       node->location,
                       "cannot cast '%s' to '%s'",
                       semantic_type_get_name(source_type)
                           ? semantic_type_get_name(source_type) : "?",
                       semantic_type_get_name(target_type)
                           ? semantic_type_get_name(target_type) : "?");
  ctx->error_count++;
  return _eval_error_val(eval);
}
...
void builtin_table_init_cast(builtin_table_t table, struct checker *ctx) {
  ...
  semantic_type_t cast_type = semantic_type_create_function(
      ctx->allocator, t_param, params, false);
