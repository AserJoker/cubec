...
static type_t  _array_vtable_type_clone(vm_t vm, type_t self);
...
static vtable_t _make_array_vtable(void) {
  return (vtable_t){
      .clone        = _array_clone,
      .type_clone   = _array_vtable_type_clone,
      .equal        = _array_equal,
      .extends      = NULL,
...
static void _array_type_init(void *self, allocator_t allocator, void *arg) {
  ...
  at->element_type = init->element_type;
  at->count        = init->count;
}
...
array_type_t array_type_create(allocator_t allocator, type_t element_type,
  ...
  size_t name_len = snprintf(NULL, 0, "[%llu]%s",
  ...
  array_type_init_t init = {
      ...
      .name         = name,
      .size         = count * type_get_size(element_type),
...
static type_t _array_vtable_type_clone(vm_t vm, type_t self) {
  array_type_t at = (array_type_t)self;
  /* recursively clone element type into current scope */
  type_t cloned_elem = value_type_clone(vm, at->element_type);
  /* create new array_type_t in current scope */
  value_t type_val = vm_create_array_type_value(vm, cloned_elem,
                                                  at->count, at->base.mut);
...
static value_t _array_equal(vm_t vm, value_t a, value_t b) {
  ...
  if (at->count != bt->count || at->element_type != bt->element_type)
...
static value_t _array_type_equal(vm_t vm, type_t a, type_t b) {
  ...
  /* fallback: same pointer = equal */
  return create_bool_value(vm, aa->element_type == ab->element_type);
}
...
static value_t _array_type_extends(vm_t vm, type_t sub, type_t super) {
  ...
  if (sub_at->count != super_at->count)
    return create_bool_value(vm, false);
  ...
  return create_bool_value(vm, sub_at->element_type == super_at->element_type);
}
...
static value_t _array_safe_cast(vm_t vm, value_t self, type_t to) {
  ...
  if (from_at->element_type != to_at->element_type)
...
static value_t _array_assignment(vm_t vm, value_t lvalue, value_t rvalue) {
  ...
  if (lat->count != rat->count || lat->element_type != rat->element_type)
