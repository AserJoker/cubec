...
static void _report_error(vm_t vm, node_t node, const char *fmt, ...) {
  location_t loc = {0};
  if (node)
  ...
  va_end(args);
  diagnostic_list_push(vm_get_diagnostics(vm), DIAGNOSTIC_ERROR, loc, "%s", buf);
}

/** Insert a name into a scope's name table. Returns true on success. */
static bool _scope_insert_name(vm_t vm, scope_t scope, node_t node,
                               const char *name_str, value_t ref) {
  name_t existing = (name_t)strmap_find(scope->names, name_str);
  if (existing) {
    _report_error(vm, node, "duplicate declaration of '%s'", name_str);
    return false;
  }
...
static void _module_export_name(vm_t vm, module_t mod, node_t node,
                                const char *name_str) {
  name_t existing = (name_t)strmap_find(mod->exports, name_str);
  if (existing) {
    _report_error(vm, node, "duplicate export of '%s'", name_str);
    return;
  }
  name_t scope_name = (name_t)strmap_find(mod->root_scope->names, name_str);
  if (!scope_name) {
    _report_error(vm, node, "cannot export undeclared name '%s'", name_str);
    return;
  }
...
static module_t _check_export_scope(vm_t vm, scope_t scope,
                                    node_t node, bool is_export) {
  module_t mod = (scope->kind == SCOPE_MODULE && scope->owner)
                     ? (module_t)scope->owner
                     : NULL;
  if (is_export && !mod)
    _report_error(vm, node, "'export' can only be used at module scope");
  return mod;
}
...
static void _collect_var_declaration(vm_t vm, scope_t scope,
                                     node_t stmt) {
  cubec_statement_declaration_t decl = (cubec_statement_declaration_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, decl->is_export);
  if (!decl->declarator)
    return;
  ...
  if (!_scope_insert_name(vm, scope, stmt, name_str, NULL))
    return;
  if (mod && decl->is_export)
    _module_export_name(vm, mod, stmt, name_str);
}

static void _collect_function_declaration(vm_t vm, scope_t scope,
                                         node_t stmt) {
  ...
  module_t mod = _check_export_scope(vm, scope, stmt, func->is_export);
...
static void _collect_struct_declaration(vm_t vm, scope_t scope,
                                       node_t stmt) {
  cubec_statement_struct_t s = (cubec_statement_struct_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, s->is_export);
...
static void _collect_union_declaration(vm_t vm, scope_t scope,
                                      node_t stmt) {
  cubec_statement_union_t u = (cubec_statement_union_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, u->is_export);
...
static void _collect_enum_declaration(vm_t vm, scope_t scope,
                                     node_t stmt) {
  cubec_statement_enum_t e = (cubec_statement_enum_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, e->is_export);
...
static void _collect_interface_declaration(vm_t vm, scope_t scope,
                                          node_t stmt) {
  cubec_statement_interface_t iface = (cubec_statement_interface_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, iface->is_export);
...
static void _collect_cunion_declaration(vm_t vm, scope_t scope,
                                       node_t stmt) {
  cubec_statement_cunion_t cu = (cubec_statement_cunion_t)stmt;
  const char *name_str = _get_identifier_name(cu->name);
  if (!name_str)
    return;
  _scope_insert_name(vm, scope, stmt, name_str, NULL);
}

static void _collect_type_alias_declaration(vm_t vm, scope_t scope,
                                           node_t stmt) {
  cubec_statement_declaration_type_t t =
      (cubec_statement_declaration_type_t)stmt;
  module_t mod = _check_export_scope(vm, scope, stmt, t->is_export);
...
static void _collect_import_statement(vm_t vm, scope_t scope,
                                     node_t stmt) {
  if (scope->kind != SCOPE_MODULE) {
    _report_error(vm, stmt, "'import' can only be used at module scope");
    return;
  }
  ...
  if (!mod_name) {
    _report_error(vm, stmt, "import statement is missing module name");
    return;
  }
  if (!mod_path) {
    _report_error(vm, stmt, "import statement is missing module path");
    return;
  }
  module_t dep_mod = vm_import(vm, mod_path);
  if (!dep_mod) {
    _report_error(vm, stmt, "cannot import module '%s' from '%s'",
                  mod_name, mod_path);
    return;
  }
  name_collector_run(vm, dep_mod);
  /* Phase 1: ref=NULL — will be filled during definition collection (phase 2). */
  _scope_insert_name(vm, scope, stmt, mod_name, NULL);
}

static void _collect_export_statement(vm_t vm, scope_t scope,
                                     node_t stmt) {
  module_t mod = (scope->kind == SCOPE_MODULE && scope->owner)
                     ? (module_t)scope->owner
                     : NULL;
  if (!mod) {
    _report_error(vm, stmt, "'export' statement can only be used at module scope");
    return;
  }
  cubec_statement_export_t exp = (cubec_statement_export_t)stmt;
  const char *mod_path = _get_string_value(exp->path);
  if (!mod_path) {
    _report_error(vm, stmt, "export statement is missing module path");
    return;
  }
  module_t dep_mod = vm_import(vm, mod_path);
  if (!dep_mod) {
    _report_error(vm, stmt, "cannot export from module '%s'", mod_path);
    return;
  }
  name_collector_run(vm, dep_mod);
  if (exp->is_star) {
    ...
    for (size_t i = 0; i < name_count; i++) {
      ...
      if (!dep_name) {
        _report_error(vm, name_node, "'%s' is not exported by '%s'",
                      exp_name, mod_path);
        continue;
      }
...
static void _collect_statement(vm_t vm, scope_t scope, node_t stmt,
                               bool is_static_scope) {
  switch (stmt->kind) {
  case CUBEC_NODE_STATEMENT_DECLARATION:
    if (is_static_scope)
      _collect_var_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_FUNCTION:
    _collect_function_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_STRUCT:
    _collect_struct_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_UNION:
    _collect_union_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_ENUM:
    _collect_enum_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_INTERFACE:
    _collect_interface_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_CUNION:
    _collect_cunion_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_DECLARATION_TYPE:
    _collect_type_alias_declaration(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_IMPORT:
    _collect_import_statement(vm, scope, stmt);
    break;
  case CUBEC_NODE_STATEMENT_EXPORT:
    _collect_export_statement(vm, scope, stmt);
    break;
  default:
    break;
  }
...
void name_collector_run(vm_t vm, module_t mod) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    _collect_statement(vm, scope, stmt, is_static_scope);
  }
