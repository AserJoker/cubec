...
semantic_type_t _check_assign_generic_lhs(context_t ctx, node_t expr,
  ...
  if (host_type->impl->kind == TYPE_ARRAY ||
      host_type->impl->kind == TYPE_SLICE) {
...
semantic_type_t _check_generic_ident_callee(context_t ctx, node_t expr) {
  ...
  if (sym && sym->kind == SYMBOL_TYPE && sym->type.type) {
    ...
    if (template_type->impl->kind == TYPE_GENERIC_INSTANCE) {
      strmap_t dummy = _resolve_generic_type_bindings_pack(
          ctx, gi->arguments, sym->type.generic_params);
      allocator_free(ctx->allocator, &dummy);
      return template_type;
    }

    strmap_t type_bindings = _resolve_generic_type_bindings_pack(
        ctx, gi->arguments, sym->type.generic_params);
    if (strmap_get_size(type_bindings) == 0 &&
        vec_get_size(gi->arguments) > 0) {
      allocator_free(ctx->allocator, &type_bindings);
      return ctx->error_type;
    }
    _check_generic_param_constraints(ctx, sym->type.generic_params,
                                     type_bindings, expr);
    return _instantiate_type(ctx, template_type, type_bindings, expr);
  }

  if (sym && sym->kind == SYMBOL_FUNCTION && sym->function.type) {
    strmap_t type_bindings_fn = _resolve_generic_type_bindings_pack(
        ctx, gi->arguments, sym->function.generic_params);
    if (strmap_get_size(type_bindings_fn) == 0 &&
        vec_get_size(gi->arguments) > 0) {
    ...
    for (size_t i = 0; i < gp_count; i++) {
      ...
      if (gp_name && !strmap_find(type_bindings_fn, gp_name)) {
        ...
        if (gp_node && gp_node->is_rest &&
          ...
          semantic_type_t empty_pack =
              semantic_type_create_generic_pack(ctx->allocator, gp_name);
    ...
    _check_generic_param_constraints(ctx, sym->function.generic_params,
                                     type_bindings_fn, expr);
    semantic_type_t inst_result =
        _instantiate_function(ctx, sym, type_bindings_fn, expr);
  ...
  if (host_type->instance_methods) {
    ...
    /* If __set__ exists (even without __get__), the type supports indexing.
       Return the value parameter type of __set__ so assignment type-checking
       works. */
    if (set_method) {
...
void _check_init_list_named_fields(context_t ctx, node_t expr,
  ...
  for (size_t i = 0; i < icount; i++) {
    ...
    if (!fsym) {
      diagnostic_list_push(ctx->diagnostics, DIAGNOSTIC_ERROR, item->location,
      ...
      if (vt->impl->kind != TYPE_ERROR &&
        ...
        diagnostic_list_push(
            ctx->diagnostics, DIAGNOSTIC_ERROR, item->location,
            "cannot initialize field '%s' of type '%s' with '%s'", fname,
            fsym->field.type->name ? fsym->field.type->name : "<anonymous>",
...
void _check_init_list_positional(context_t ctx, node_t expr, semantic_type_t t,
                                 vec_t fields, size_t fcount, size_t icount,
                                 vec_t items) {
  size_t field_idx = 0;
  for (size_t i = 0; i < icount; i++) {
    node_t item = (node_t)vec_get(items, i);
    ...
    if (item && item->kind == CUBEC_NODE_EXPRESSION_SPREAD) {
      ...
      if (spread_type && spread_type->impl &&
          spread_type->impl->kind == TYPE_GENERIC_PACK) {
          ...
          if (et && fsym && fsym->field.type) {
            if (et->impl->kind != TYPE_ERROR &&
              ...
              diagnostic_list_push(
                  ...
                  fsym->field.type->name ? fsym->field.type->name
                                         : "<anonymous>",
      ...
      /* If spread_type is not TYPE_GENERIC_PACK, it's already been type-checked
         by _check_expression → _check_expr_spread, which returns the inner
         type. In that case, we skip expansion and treat it as a single element.
       */
    } else {
...
semantic_type_t _check_binary_arithmetic(context_t ctx, node_t expr,
                                         const char *op, semantic_type_t lt,
                                         semantic_type_t rt) {
...
/* _check_func_params replaced by _resolve_func_param_types +
   _register_func_params_from_info in checker_func_util.c */
