...
static bool _is_comptime_numeric(comptime_value_t v) {
  return v &&
         (v->kind == COMPTIME_VALUE_INT || v->kind == COMPTIME_VALUE_FLOAT);
}
...
comptime_value_t _comptime_eval_binary(comptime_eval_t eval, context_t ctx,
  ...
  if (!bin->left) {
    ...
    if (strcmp(op, "!") == 0)
      result = comptime_value_create_bool(
          eval->allocator, !comptime_value_is_truthy(rv), rv->type);
    else if (strcmp(op, "-") == 0) {
      if (rv->kind == COMPTIME_VALUE_INT)
        result = comptime_value_create_int(
            eval->allocator, -rv->int_val.s, (uint64_t)(-rv->int_val.s),
            rv->int_val.width, rv->int_val.is_signed, rv->type);
      else if (rv->kind == COMPTIME_VALUE_FLOAT)
        result =
            comptime_value_create_float(eval->allocator, -rv->float_val.value,
                                        rv->float_val.width, rv->type);
    } else if (strcmp(op, "~") == 0) {
      if (rv->kind == COMPTIME_VALUE_INT)
        result = comptime_value_create_int(
            eval->allocator, (int64_t)(~rv->int_val.u), ~rv->int_val.u,
            rv->int_val.width, rv->int_val.is_signed, rv->type);
    }
  ...
  if (!_is_comptime_numeric(rv) && rv->type && rv->type->instance_methods) {
    ...
    for (size_t i = 0; i < mc; i++) {
      struct symbol *s =
          (struct symbol *)vec_get(rv->type->instance_methods, i);
      if (s && s->name && strcmp(s->name, "__value__") == 0 &&
          s->kind == SYMBOL_FUNCTION) {
  ...
  if ((lv->kind == COMPTIME_VALUE_POINTER ||
       rv->kind == COMPTIME_VALUE_POINTER) &&
  ...
  if (strcmp(op, "+") == 0) {
    ...
    if (lv->kind == COMPTIME_VALUE_STRING &&
        rv->kind == COMPTIME_VALUE_STRING) {
    ...
    } else if (lv->kind == COMPTIME_VALUE_FLOAT ||
               rv->kind == COMPTIME_VALUE_FLOAT)
    ...
    if (rv->kind == COMPTIME_VALUE_INT &&
      ...
      diagnostic_list_push(ctx->diagnostics, DIAGNOSTIC_ERROR, node->location,
                           "division by zero");
      ctx->error_count++;
    } else if (lv->kind == COMPTIME_VALUE_INT &&
               rv->kind == COMPTIME_VALUE_INT) {
      int64_t s = lv->int_val.is_signed
                      ? lv->int_val.s / rv->int_val.s
  ...
  /* Comparison */
    ...
    result = comptime_value_create_bool(
        eval->allocator, comptime_value_equals(lv, rv), lv->type);
  else if (strcmp(op, "!=") == 0)
    result = comptime_value_create_bool(
        eval->allocator, !comptime_value_equals(lv, rv), lv->type);
  ...
  /* Relational */
    ...
    if (lv->kind == COMPTIME_VALUE_INT && rv->kind == COMPTIME_VALUE_INT) {
      if (lv->int_val.is_signed && rv->int_val.is_signed) {
        if (strcmp(op, "<") == 0)
          bresult = lv->int_val.s < rv->int_val.s;
        if (strcmp(op, "<=") == 0)
          bresult = lv->int_val.s <= rv->int_val.s;
        if (strcmp(op, ">") == 0)
          bresult = lv->int_val.s > rv->int_val.s;
        ...
        if (strcmp(op, "<") == 0)
          bresult = lv->int_val.u < rv->int_val.u;
        if (strcmp(op, "<=") == 0)
          bresult = lv->int_val.u <= rv->int_val.u;
        if (strcmp(op, ">") == 0)
          bresult = lv->int_val.u > rv->int_val.u;
    ...
    result =
        comptime_value_create_bool(eval->allocator, bresult, ctx->builtin_bool);
  }
  ...
  /* Bitwise */
           ...
           rv->kind == COMPTIME_VALUE_INT)
    result = comptime_value_create_int(
        eval->allocator, (int64_t)(lv->int_val.u & rv->int_val.u),
        lv->int_val.u & rv->int_val.u, lv->int_val.width, lv->int_val.is_signed,
        lv->type);
  else if (strcmp(op, "|") == 0 && lv->kind == COMPTIME_VALUE_INT &&
           rv->kind == COMPTIME_VALUE_INT)
    result = comptime_value_create_int(
    ...
    result = comptime_value_create_int(
        eval->allocator, (int64_t)(lv->int_val.u << rv->int_val.u),
        lv->int_val.u << rv->int_val.u, lv->int_val.width,
        lv->int_val.is_signed, lv->type);
  else if (strcmp(op, ">>") == 0 && lv->kind == COMPTIME_VALUE_INT &&
           rv->kind == COMPTIME_VALUE_INT) {
    ...
    result = comptime_value_create_int(eval->allocator, (int64_t)shifted,
                                       shifted, lv->int_val.width,
                                       lv->int_val.is_signed, lv->type);
  }

  /* extends: compile-time constraint check — returns bool */
  else if (strcmp(op, "extends") == 0) {
    semantic_type_t type_arg =
        (lv->kind == COMPTIME_VALUE_TYPE) ? lv->type_val : lv->type;
    semantic_type_t constraint =
        (rv->kind == COMPTIME_VALUE_TYPE) ? rv->type_val : rv->type;
    if (type_arg && constraint) {
      bool satisfies = _check_constraint_silent(ctx, type_arg, constraint);
      result = comptime_value_create_bool(eval->allocator, satisfies,
                                          ctx->builtin_bool);
    }
  }
  /* is: union variant check — returns bool */
    ...
    if (lv->kind == COMPTIME_VALUE_COMPOSITE &&
      ...
      if (target_type && target_type->impl->kind != TYPE_ERROR) {
        ...
        result = comptime_value_create_bool(eval->allocator, is_match,
                                            ctx->builtin_bool);
      }
    } else {
      /* Not a union value — error already reported by checker */
      result =
          comptime_value_create_bool(eval->allocator, false, ctx->builtin_bool);
    }
  }
