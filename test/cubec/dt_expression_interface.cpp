...
class dt_expression_interface : public CubecTest {
protected:
...
/* Simple: interface { } */
TEST_F(dt_expression_interface, simple_empty) {
  ...
  EXPECT_EQ(node->kind, CUBEC_NODE_EXPRESSION_INTERFACE);
...
TEST_F(dt_expression_interface, with_method) {
...
TEST_F(dt_expression_interface, with_type_and_method) {
...
TEST_F(dt_expression_interface, generic) {
...
TEST_F(dt_expression_interface, generic_multi) {
...
TEST_F(dt_expression_interface, pointer_to_interface) {
  ...
  EXPECT_EQ(ptr->type->kind, CUBEC_NODE_EXPRESSION_INTERFACE);
...
TEST_F(dt_expression_interface, slice_of_interface) {
  ...
  EXPECT_EQ(slice->type->kind, CUBEC_NODE_EXPRESSION_INTERFACE);
...
TEST_F(dt_expression_interface, const_interface) {
  ...
  cubec_expression_qualifier_t qual =
      (cubec_expression_qualifier_t)node;
  ASSERT_NE(qual->type, nullptr);
  EXPECT_EQ(qual->type->kind, CUBEC_NODE_EXPRESSION_INTERFACE);
...
TEST_F(dt_expression_interface, consume_all_tokens) {
  const char *source = "interface { }";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
...
TEST_F(dt_expression_interface, non_interface_returns_null) {
  const char *source = "i32";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);

  size_t position = 0;
  node_t node = read_expression_interface(ctx, tokens, &position, "test.cubec");
...
TEST_F(dt_expression_interface, clone) {
  const char *source = "interface { func next(self): Item; }";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
  ...
  EXPECT_EQ(cloned->kind, CUBEC_NODE_EXPRESSION_INTERFACE);

  cubec_expression_interface_t copy =
      (cubec_expression_interface_t)cloned;
...
TEST_F(dt_expression_interface, move) {
  const char *source = "interface { func next(self): Item; }";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
  ...
  EXPECT_EQ(moved->kind, CUBEC_NODE_EXPRESSION_INTERFACE);

  cubec_expression_interface_t result =
      (cubec_expression_interface_t)moved;
...
TEST_F(dt_expression_interface, via_read_atom) {
  const char *source = "interface { func bar(): i32; }";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
...
TEST_F(dt_expression_interface, via_read_expression) {
  const char *source = "interface { }";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
