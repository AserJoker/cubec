...
class dt_expression_assert : public CubecTest {
protected:
...
TEST_F(dt_expression_assert, parse_assert) {
  const char *source = "x.!";
  ...
  EXPECT_EQ(node->kind, CUBEC_NODE_EXPRESSION_ASSERT);

  allocator_free(allocator, &node);
  allocator_free(allocator, &tokens);
}

TEST_F(dt_expression_assert, write_basic) {
  const char *source = "x.!";
  vec_t tokens = resolve_token_list(ctx, "test.cubec", source);
  ASSERT_NE(tokens, nullptr);
  size_t position = 0;
