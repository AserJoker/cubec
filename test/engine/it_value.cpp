...
TEST_F(it_value, create_and_accessors) {
  ...
  *data = 42;
  ...
  value_t v = value_create(allocator, i32_type, data, true, 0);
...
TEST_F(it_value, shadow_value) {
  type_t i32_type = _make_i32_type();
  value_t v = value_create(allocator, i32_type, NULL, false, 0);
...
TEST_F(it_value, dispose_no_vtable_no_crash) {
  ...
  value_t v = value_create(allocator, void_type, NULL, false, 0);
  value_dispose(v, allocator);
  delete_allocator(allocator);
}
...
TEST_F(it_value, type_kind_type_bootstrap) {
  /* TYPE_KIND_TYPE: value.type and value.data point to same type_t */
  type_t meta = (type_t)allocator_alloc(allocator, sizeof(struct _type_t));
  meta->kind = TYPE_KIND_TYPE;
  meta->name = "Type";
  meta->size = sizeof(struct _type_t);
  meta->align = 8;
  meta->vtable = (vtable_t){.clone = NULL, .dispose = NULL};

  value_t v = value_create(allocator, meta, meta, true, 0);
  EXPECT_EQ(value_get_type(v), meta);
  EXPECT_EQ(value_get_data(v), meta);
  EXPECT_EQ(value_get_type(v), value_get_data(v));
  EXPECT_TRUE(value_is_own(v));

  /* own=true but vtable.dispose=NULL, so just free the value shell */
  value_dispose(v, allocator);
  delete_allocator(allocator);
}
