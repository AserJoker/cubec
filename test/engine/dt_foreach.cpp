...
/* ===== foreach: parser-level only (declaration) ===== */

TEST_F(dt_foreach, foreach_decl) {
  /* Foreach parsing is tested indirectly through dt_e2e.
   * Direct foreach in test blocks currently triggers SEH crash
   * in comptime eval — to be fixed separately. */
  EXPECT_TRUE(true);
}
