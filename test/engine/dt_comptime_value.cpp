...
TEST_F(dt_comptime_value, create_bool) {
  context_t ctx = context_create(allocator);
  comptime_value_t t =
      comptime_value_create_bool(allocator, true, ctx->builtin_bool);
  comptime_value_t f =
      comptime_value_create_bool(allocator, false, ctx->builtin_bool);
  ASSERT_NE(t, nullptr);
...
TEST_F(dt_comptime_value, create_int_signed) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_int(allocator, -42, 0, 32, true, ctx->builtin_i32);
  ASSERT_NE(v, nullptr);
...
TEST_F(dt_comptime_value, create_int_unsigned) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_int(allocator, 0, 300, 16, false, ctx->builtin_u16);
  ASSERT_NE(v, nullptr);
  EXPECT_EQ(v->kind, COMPTIME_VALUE_INT);
...
TEST_F(dt_comptime_value, int_truthy) {
  context_t ctx = context_create(allocator);
  comptime_value_t zero =
      comptime_value_create_int(allocator, 0, 0, 32, true, ctx->builtin_i32);
  comptime_value_t one =
      comptime_value_create_int(allocator, 1, 1, 32, true, ctx->builtin_i32);
...
TEST_F(dt_comptime_value, create_float) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_float(allocator, 3.14, 64, ctx->builtin_f64);
  ASSERT_NE(v, nullptr);
...
TEST_F(dt_comptime_value, float_truthy) {
  context_t ctx = context_create(allocator);
  comptime_value_t zero =
      comptime_value_create_float(allocator, 0.0, 64, ctx->builtin_f64);
  comptime_value_t nonzero =
      comptime_value_create_float(allocator, 1.5, 64, ctx->builtin_f64);
  EXPECT_FALSE(comptime_value_is_truthy(zero));
...
TEST_F(dt_comptime_value, create_literal_char) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_literal_char(allocator, 'A', ctx->builtin_char);
  ASSERT_NE(v, nullptr);
...
TEST_F(dt_comptime_value, create_literal_string) {
  context_t ctx = context_create(allocator);
  comptime_value_t v = comptime_value_create_literal_string(allocator, "hello",
                                                            ctx->builtin_str);
  ASSERT_NE(v, nullptr);
...
TEST_F(dt_comptime_value, create_pointer) {
  ...
  semantic_type_t ptr_type =
      NULL; /* simplified — real ptr types need resolver */
...
TEST_F(dt_comptime_value, equals_int) {
  context_t ctx = context_create(allocator);
  comptime_value_t a =
      comptime_value_create_int(allocator, 42, 42, 32, true, ctx->builtin_i32);
  comptime_value_t b =
      comptime_value_create_int(allocator, 42, 42, 32, true, ctx->builtin_i32);
  EXPECT_TRUE(comptime_value_equals(a, b));
  allocator_free(allocator, &a);
...
TEST_F(dt_comptime_value, clone_int) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_int(allocator, -99, 0, 32, true, ctx->builtin_i32);
...
TEST_F(dt_comptime_value, clone_string) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_literal_string(allocator, "abc", ctx->builtin_str);
  comptime_value_t c = comptime_value_clone(allocator, v);
  ASSERT_NE(c, nullptr);
...
TEST_F(dt_comptime_value, as_f64_from_int) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_int(allocator, 3, 3, 32, true, ctx->builtin_i32);
...
TEST_F(dt_comptime_value, as_f64_from_float) {
  context_t ctx = context_create(allocator);
  comptime_value_t v =
      comptime_value_create_float(allocator, 2.5, 64, ctx->builtin_f64);
