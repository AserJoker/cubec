...
TEST_F(test_enum, normal) {
  ...
  ASSERT_TRUE(node_location_is(pub, "pub"));
  ASSERT_NE(identifier, nullptr);
  ASSERT_TRUE(node_location_is(identifier, "Color"));
  ...
  ASSERT_TRUE(node_location_is(name, "Red"));
  ASSERT_EQ(value, nullptr);
  option = ast_get_item(options, 1);
  ASSERT_NE(option, nullptr);
  name = ast_get_child(option, "identifier");
  value = ast_get_child(option, "value");
  ASSERT_TRUE(node_location_is(name, "Green"));
  ASSERT_TRUE(node_location_is(value, "2"));
